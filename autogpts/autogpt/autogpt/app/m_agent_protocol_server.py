...
from autogpt.core.model import ChatRequestBody
...
from autogpt.core.middlewares import MTAgentMiddleware
from .ui import demo
from autogpt.core.routes.api import m_router
from autogpt.core.memory.db import SupaDB
...
class MTAgentProtocolServer:
    def __init__(
        self,
        app_config: Config,
        database: AgentDB | SupaDB,
    ...
    async def start(self, port: int = 8000, router: APIRouter = m_router):
        ...
        agent = configure_agent_with_state(
            AgentSettings(name="", description=""),
            self.app_config,
            self.llm_provider,
            agent_type="MTAgent"
        )
        app.add_middleware(MTAgentMiddleware, protocol=self, agent=agent)
    ...
    async def create_agent(self, chat_request: ChatRequestBody) -> Task:
        """
        Create a task for the agent.
        """
        # task = await self.db.create_task(
        #     input=chat_request.input,
        #     additional_input=chat_request.additional_input,
        # )

        logger.debug(f"[ {__file__} ] | Creating agent for task: '{chat_request.input}'")
        task_agent = await generate_agent_for_task(
            task=chat_request.input,
            app_config=self.app_config,
            llm_provider=self._get_task_llm_provider(),
        )
        logger.debug(f"[ {__file__} ] | history: {task_agent.event_history}")

        # agent_id = task_agent.state.agent_id = task_agent_id(task.task_id)
        # logger.debug(f"[ {__file__} ] | New agent ID: {agent_id}")
    ...
    async def get_task(self, task_id: str) -> Task:
        ...
        Get a task by ID.
        """
        logger.debug(f"[ {__file__} ] | Getting task with ID: {task_id}...")
    ...
    async def execute_step(self, task_id: str, step_request: StepRequestBody) -> Step:
        ...
        logger.debug(f"[ {__file__} ] | Creating a step for task with ID: {task_id}...")
        ...
        logger.debug(f"[ {__file__} ] | step_request: {step_request}")
        ...
        if execute_command:
            ...
            if step.is_last and execute_command == finish.__name__:
                ...
                logger.debug(f"[ {__file__} ] | Update step with FINISH {finish.__name__}")
                return step

            if execute_command == ask_user.__name__:  # HACK
                execute_result = ActionSuccessResult(outputs=user_input)
                logger.debug(f"[ {__file__} ] | Last execution succeed")
                ...
                # Execute previously proposed action
                logger.debug(f"[ {__file__} ] | Executing last proposed action")
                execute_result = await agent.execute(
                    command_name=execute_command,
                    command_args=execute_command_args,
                )
                logger.debug(f"[ {__file__} ] | Last proposed action finished")
            else:
                assert user_input
                logger.debug(f"[ {__file__} ] | Execute 'human_feedback' ? ")
                execute_result = await agent.execute(
        ...
        try:
            ...
            logger.debug(f"[ {__file__} ] | Propose action with AI output: {raw_output}")
        ...
        logger.debug(f"[ {__file__} ] | step output: {output}")
        ...
        logger.debug(f"[ {__file__} ] | step additional output: {output}")
    ...
    def _get_task_llm_provider(self) -> ChatModelProvider:
        """
    ...
    def get_llm_provider(self) -> ChatModelProvider:
        
        return copy.deepcopy(self.llm_provider)
